...
import { AlertTriangle, Power, Zap, Activity, TrendingUp, BarChart3, GitBranch } from 'lucide-react';
...
const TendonDrivenHandGUI = () => {
  ...
  const HandVisualization = () => {
    ...
    useEffect(() => {
      ...
      const drawSegmentUp = (w, len, fill) => {
        ctx.fillStyle = fill;
        // rectangle whose bottom is at the joint (0,0) and goes upward
        ctx.fillRect(-w / 2, -len, w, len);
        // move origin to the end of the segment (next joint location)
        ctx.translate(0, -len);
      };
      // IMPORTANT: attachment points must actually sit on/near the palm.
      // Top edge is palmY, right edge is palmX + palmWidth.
      const top = palmY;
      const left = palmX;
      const right = palmX + palmWidth;
      // Fingers attach along the top edge; thumb attaches on left side
      const fingers = [
        {
          name: "thumb",
          attachX: left - 2,             // just outside left edge
          attachY: palmY + 70,           // thumb attaches on side, not top
          joints: joints.thumb,
          baseAngle: -60,                // points up-left
        },
        {
          name: "index",
          ...
          attachY: top + 2,
          joints: joints.index,
          baseAngle: -10,                // slight splay
        },
        {
          name: "middle",
          attachX: left + palmWidth * 0.50,
          attachY: top + 0,
          joints: joints.middle,
          baseAngle: 0,
        },
        {
          name: "ring",
        ...
        {
          name: "pinky",
          attachX: right - 4,            // keep it on the palm edge (NOT past it)
          attachY: top + 8,
          joints: joints.pinky,
          baseAngle: 18,
        },
      ...
      fingers.forEach((finger, idx) => {
        ctx.save();
        const servoColor = servos?.[idx]?.color || "#63b3ed";
        // Move to fixed attachment point
        ctx.translate(finger.attachX, finger.attachY);
        // Orient finger "mostly up" (we draw segments upward by default)
        ...
        // NOTE:
        // Positive canvas rotation is clockwise. With our finger pointing "up",
        // flexion (curling toward the palm) is typically clockwise for a right-hand view.
        // If your flex direction looks reversed, flip the sign: use (-angle) below.
  ...
  const TimeSeriesPlot = ({ title, dataKey, ylabel, colors }) => {
    ...
    useEffect(() => {
      ...
      servos.forEach((servo, servoIdx) => {
        ...
        timeSeriesData.forEach((point, idx) => {
          ...
          const x = padding.left + (plotWidth * idx / (timeSeriesData.length - 1 || 1));
    ...
    }, [timeSeriesData, dataKey, title, ylabel, colors]);

    return <canvas ref={canvasRef} width={600} height={300} className="border border-gray-300 rounded" />;
  };

  // Mapping Curve Plot
  const MappingPlot = ({ title, xLabel, yLabel, dataPoints, xRange, yRange, currentServo, color }) => {
    const canvasRef = useRef(null);
    ...
    useEffect(() => {
      ...
      // Current position marker for selected servo
      if (currentServo !== undefined && servos[currentServo]) {
        const servo = servos[currentServo];
        const currentX = servo.pwm;
        const currentY = servo.angle;
    ...
    }, [title, xLabel, yLabel, dataPoints, xRange, yRange, servos, currentServo, color]);
  ...
  const MappingPage = () => {
    ...
    const allFingerData = fingerTypes.map(type => {
      ...
      for (let delta = -20; delta <= 20; delta += 0.5) {
        const joints = tendonToJoints(delta, type);
        data.push({ 
          delta, 
          mcp: joints.mcp, 
          pip: joints.pip || joints.ip, // IP for thumb
          dip: joints.dip || 0,
          ip: joints.ip || 0
        });
      }
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-white rounded-lg shadow-md p-4">
            ...
            <MappingPlot 
              title={`PWM → Servo Angle (${servos[selectedFinger].name})`}
              xLabel="PWM Command (μs)"
              yLabel="Servo Angle (degrees)"
              dataPoints={pwmToAngleData}
              xRange={[PWM_MIN, PWM_MAX]}
              yRange={[0, 180]}
              currentServo={selectedFinger}
              color={servos[selectedFinger].color}
            />
            <div className="mt-3 p-3 bg-gray-50 rounded">
              <p className="text-xs text-gray-700">
                <strong>Linear Transfer Function:</strong> PWM duty cycle directly controls servo position.
                Current position: <span className="font-mono font-bold">{servos[selectedFinger].pwm}μs → {servos[selectedFinger].angle.toFixed(1)}°</span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-4">
            <div className="mb-3">
              <h3 className="text-sm font-semibold text-gray-700">Current: {servos[selectedFinger].name}</h3>
            </div>
            
            <MappingPlot 
              title={`Servo Angle → Tendon Δ (${servos[selectedFinger].name})`}
            ...
            <div className="mt-3 p-3 bg-gray-50 rounded">
              <p className="text-xs text-gray-700">
                ...
                Current: <span className="font-mono font-bold">{servos[selectedFinger].angle.toFixed(1)}° → {angleToTendonDelta(servos[selectedFinger].angle).toFixed(2)}mm</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          ...
          <TendonToJointPlot 
            data={allFingerData[selectedFinger]} 
            fingerType={fingerTypes[selectedFinger]}
            color={servos[selectedFinger].color}
          />
          
          <div className="mt-3 p-3 bg-blue-50 rounded">
            <p className="text-sm text-gray-700 mb-2">
              <strong>Underactuated Coupling for {servos[selectedFinger].name}:</strong>
            </p>
            {fingerTypes[selectedFinger] === 'thumb' ? (
  ...
  // Tendon to Joint Coupling Plot
  const TendonToJointPlot = ({ data, fingerType, color }) => {
    const canvasRef = useRef(null);
  ...
  return (
    <div className="w-full min-h-screen bg-gray-100 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Page Content */}
        {activePage === 'control' && <ControlPage />}
        {activePage === 'timeseries' && <TimeSeriesPage />}
        {activePage === 'mapping' && <MappingPage />}
      </div>
    </div>
  );
};
